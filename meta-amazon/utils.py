...
def multi_data_loader(inputs, targets, batch_size, shuffle=True):
    ...
    for j in range(num_blocks): # data iterator
        xs, ys = [], []
        for i in range(num_domains): # 3 source domains
            ridx = np.random.choice(input_sizes[i], batch_size)
            # if np.sum(targets[i][ridx]) == 0.0 or np.sum(targets[i][ridx]) == np.float(batch_size): # must has pos and neg
            #     break
