...
def train(args):
    ...
    for i in range(num_data_sets):
        dataset = MNIST_Dataset(dataset_names[i],is_train = True,
            max_data_num =1000,transform=data_transforms[split_name[True]])
    ...
    infer_data_insts, infer_data_labels, infer_num_insts = [], [], []
    for i in range(num_data_sets):
        dataset = MNIST_Dataset(dataset_names[i],is_train = True,
            max_data_num =1000,transform=data_transforms[split_name[True]])
        ...
        infer_data_insts.append(tmp_data)
        
        infer_data_labels.append(tmp_label)
    ...
    if args.model=="mdan":
        configs = {"num_classes": 10,
               "num_epochs": args.epoch, "batch_size": args.batch_size, "lr": 1e-1, "mu": args.mu, "num_domains":
                   3, "mode": args.mode, "gamma": 10.0, "verbose": args.verbose, "lambda": 0.01, 'margin':0.1, 'dropout':0,'k':2,'alpha':0.2, 'device':device, 
                   'nheads':1,"gat_hidden_layers": [512, 512],}
        ...
        for i in range(num_data_sets):
            ...
            for j in range(num_data_sets):
                if j != i:
                    ...
                    infer_source_insts.append(infer_data_insts[j][:,:,:,:].numpy().astype(np.float32))
                    infer_source_labels.append(infer_data_labels[j][:].ravel().astype(np.int64))
            # Build target instances.
            
            target_idx = i
            target_dataset = MNIST_Dataset(dataset_names[i],is_train = True,
                      max_data_num =1000,transform=data_transforms[split_name[True]])
            tmp_data ,tmp_label = [],[]
            ...
            preds_labels = torch.max(model.inference(infer_source_insts,target_insts), 1)[1].cpu().data.squeeze_()
...
if __name__ =='__main__':
    ...
    parser.add_argument("-b", "--batch_size", help="Batch size during training", type=int, default=5)
